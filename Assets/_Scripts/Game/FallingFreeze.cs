    ...
    protected override void CollisionEnterCode(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("playersbat"))
        {
            Messenger<ParticleTypes, Vector3>.Broadcast(GlobalEvents.SpawnParticleEffect,
                ParticleTypes.PowerupCollected, collision.contacts[0].point, MessengerMode.DONT_REQUIRE_LISTENER);
        ...
        if (collision.gameObject.CompareTag("deadzone"))
        {
            ...
            Messenger<ParticleTypes, Vector3>.Broadcast(GlobalEvents.SpawnParticleEffect, ParticleTypes.PowerupLost,
                powerupLostEffectPosition, MessengerMode.DONT_REQUIRE_LISTENER);
            Disable();
        }
    }
